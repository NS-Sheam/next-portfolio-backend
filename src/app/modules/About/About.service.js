...
const getAllAbout = async (query) => {
  ...
  const resultQuery = new QueryBuilder(About.find(), query)
    .search(AboutSearchableFields)
    .filter()
    .sort()
    .fields()
    .paginate()
...
const updateAbout = async (id, payload) => {
  const { skills, experience, education, achievements, hobbiesAndInterests, ...remaining } = payload;
  ...
  if (skills && skills.length > 0) {
    const existingSkills = about.skills.map((skill) => ({ name: skill, isDeleted: false }));
  ...
  if (experience && experience.length > 0) {
    const existingExperience = about.experience.map((exp) => ({ ...exp.toObject(), isDeleted: false }));
  ...
  if (education && education.length > 0) {
    const existingEducation = about.education.map((edu) => ({ ...edu.toObject(), isDeleted: false }));
  ...
  if (achievements && achievements.length > 0) {
    const existingAchievements = about.achievements.map((ach) => ({ ...ach.toObject(), isDeleted: false }));
  ...
  if (hobbiesAndInterests && hobbiesAndInterests.length > 0) {
    const existingHobbies = about.hobbiesAndInterests.map((hobby) => ({ name: hobby, isDeleted: false }));
  ...
  const result = await About.findByIdAndUpdate(id, remaining, { new: true, runValidators: true });
  return result;
};
