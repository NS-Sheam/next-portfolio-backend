...
const getAllBlog = async query => {
  const resultQuery = new _QueryBuilder.default(_blogModel.Blog.find().populate("author"), query).search(_blogConst.blogSearchableFields).filter().sort().fields().paginate().limit();
...
const updateBlog = async (id, file, payload) => {
  ...
  if (file) {
    ...
    const {
      secure_url
    } = await (0, _sendImageToCloudinary.sendImageToCloudinary)(imageName, imagePath);
    payload.image = secure_url;
  }
